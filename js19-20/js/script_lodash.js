...
var skillsArray = _(dataArray).map('skills').flattenDeep().sortBy().sortedUniq().value();
console.log('Массив скилов: ', skillsArray);

var nameArray = _(dataArray).map('name').sortBy('friends').value();
console.log('Массив имен: ', nameArray);

var friendsArray = _(dataArray).map('friends').flatten().map('name').uniq(dataArray).value();
console.log('Массив друзей: ', friendsArray);
